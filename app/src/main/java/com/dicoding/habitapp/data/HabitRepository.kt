...
class HabitRepository(private val habitDao: HabitDao, private val executor: ExecutorService) {
    ...
    fun getHabits(filter: HabitSortType): LiveData<PagedList<Habit>> {
        val filterable = SortUtils.getSorteredQuery(filter)
        val listData: LiveData<PagedList<Habit>>
        val config = PagedList.Config.Builder()
            .setPageSize(2)
            .build()
        val data = habitDao.getHabits(filterable)
        listData = LivePagedListBuilder(data, config).build()
